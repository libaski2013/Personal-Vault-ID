const { Reminder } = require('../db/models');

module.exports = async function reminderRoutes(fastify) {
  fastify.get('/', { onRequest: [fastify.authenticate] }, async (req) => {
    const data = await Reminder.find({ userId: req.user.userId }).sort({ dueDate: 1 });
    return { success: true, data };
  });

  fastify.post('/', { onRequest: [fastify.authenticate] }, async (req, reply) => {
    const { title, dueDate, priority } = req.body || {};
    if (!title) return reply.code(400).send({ success: false, message: 'title required' });
    const data = await Reminder.create({ userId: req.user.userId, title, dueDate, priority: priority || 'medium' });
    return reply.code(201).send({ success: true, data });
  });

  fastify.put('/:id', { onRequest: [fastify.authenticate] }, async (req, reply) => {
    const data = await Reminder.findOneAndUpdate(
      { _id: req.params.id, userId: req.user.userId },
      req.body,
      { new: true }
    );
    if (!data) return reply.code(404).send({ success: false, message: 'Reminder not found' });
    return { success: true, data };
  });

  fastify.delete('/:id', { onRequest: [fastify.authenticate] }, async (req, reply) => {
    const doc = await Reminder.findOneAndDelete({ _id: req.params.id, userId: req.user.userId });
    if (!doc) return reply.code(404).send({ success: false, message: 'Reminder not found' });
    return { success: true };
  });
};
